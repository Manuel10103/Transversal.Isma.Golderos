...
const choiceToken = (val) => {
    // Convertir el valor de la ficha a entero
    choiceBet = parseInt(val);
...
const check = () => {
	for(let i=0; i<nbrs.length; i++){
		...
		if(out==nbr){
			...
			nbrs[i].style.backgroundColor = "yellow";
			setTimeout(function() { 
	...
	if(account>=tempAccount-currentlyBet)finish.innerText = "GANADO: " + (account - (tempAccount - currentlyBet)) + " €";
	else finish.innerText = "GANADO: 0 €";
	if(currentlyBet<=0)finish.innerText = "Hagan sus apuestas";
};
...
function Roulette(containerId, options, colors, onSpinEnd){
	...
	this.spin = () => {
		...
		spinTime = 0;
		spinTimeTotal = Math.random() * 3 + 4 * 3500;
		this.rotateWheel();
	}
	...
	this.stopRotateWheel = () => {
		...
		// Actualizar el contenido del elemento HTML con clase 'resultados'
...
function ingresar() {
    var saldoInput = document.getElementById("ingresarCantidad").value;
...
function retirar() {
    var saldo = parseInt(document.getElementById("retirarCantidad").value);

    cerrarModal("modalRetirar");

    console.log("Cantidad retirada: " + saldo + "€");

    var saldoActual = parseInt(document.getElementById("Saldo").textContent);
    var compteElement = document.getElementById("compte");
